...
The code will works as expected. However, it has the following disadvantages:
...
With the client generated by MyGQL, now we can write our code like the following snippet:
...
The `countries.ts` file contains all types we found in the introspection (`countries.json`) and exports a function named `createGraphQLClient` as its default export. Now we can use this function to create our GraphQL client.

Create new a file and save it to `<root>/src/graphql/client.ts`:
...
const sendQuery = async (query: string) => {
  // In this example we will use the Fetch API. You can use whatever
  // you want, maybe axios for example.
...
// The factory function accepts a async function as its argument.
